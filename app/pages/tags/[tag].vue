...
usePageSeo({
  title: `标签：${tag.value}`,
  description: `浏览标签「${tag.value}」下的技术文章。`,
  path: `/tags/${encodeURIComponent(tag.value)}`
})
  ...
  <main>
    <section class="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
      <div class="mx-auto max-w-6xl px-5 py-14">
        <NuxtLink class="text-sm font-bold text-slate-500 hover:text-blue-700 dark:text-slate-400 dark:hover:text-blue-300" to="/tags">
          返回标签
        </NuxtLink>
        <p class="mt-6 text-sm font-bold text-blue-600 dark:text-blue-300">Tag</p>
        <h1 class="mt-3 text-4xl font-bold text-slate-950 dark:text-white">{{ tag }}</h1>
    ...
    <section class="mx-auto grid max-w-6xl gap-4 px-5 py-12">
      <div class="flex justify-end">
        <NuxtLink class="inline-flex h-10 items-center justify-center rounded-lg border border-slate-200 px-4 text-sm font-bold text-slate-600 transition hover:border-blue-200 hover:text-blue-700 dark:border-slate-700 dark:text-slate-300 dark:hover:border-blue-800 dark:hover:text-blue-300" :to="`/posts?tag=${encodeURIComponent(tag)}`">
          在文章页筛选
        </NuxtLink>
      </div>
